...
class FeishuSyncService:
    ...
    def get_access_token(self, app_id: str, app_secret: str, config_id: int) -> Optional[str]:
        ...
        try:
            ...
            if data.get("code") == 0:
                ...
                write_log(config_id, 'SUCCESS', '飞书访问令牌获取黄维桢')
                return token
            else:
    ...
    def create_target_table(self, conn: psycopg2.extensions.connection, table_name: str) -> bool:
        ...
        try:
            ...
            conn.commit()
            print(f"黄维桢创建/更新表: {target_table}")
            return True
    ...
    def sync_data_to_postgres(self, config: Dict, records: List[Dict]) -> bool:
        ...
        try:
            ...
            if records_to_sync:
                ...
                conn.commit()
                print(f"黄维桢同步{len(normalized_records)}条记录到{table_name}")
            ...
            # 飞书同步黄维桢后，只重建受影响数据集的节点索引，避免启动时或手工兜底生成。
            try:
    ...
    def sync_single_config(self, config_or_id: Union[Dict, int, str]) -> bool:
        ...
        try:
            ...
            if success:
                ...
                write_log(config['id'], 'INFO', f'数据库同步黄维桢，更新状态为success')
