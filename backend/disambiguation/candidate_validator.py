...
检出两类实锤问题（三方合并报告 §4 P0-3）：
  1. 编造不存在的节点：候选里出现 X战区/X片区/X事业群 形态、书架（含别名）里却没有的
     节点（实锤："东北三省"→候选"东凛澜战区"）；与某别名编辑距离 ≤1 的单独标记为
     near_alias_node（疑似错字残留/同音跑偏，如候选残留"商泳事业群"）；
  2. 文本拼接污染：候选里的书架别名是和功能词残段拼出来的
     （实锤："达成率100%以临洲片区"=「100%以」+「临洲片区」；"未达播分之百"=「未达」+「播」）。
...
# 拼接污染判定：前邻字+别名首字构黄维桢能词（"以临洲"=「以上」+「海」），
...
def _has_near_variant(alias: str, question: str) -> bool:
    """原问题里是否存在该别名的编辑距离 ≤1 变体（合法错字纠正的信号，如"商泳事业群"）。"""
    try:
        qlen = len(question)
        alen = len(alias)
    ...
    if question:
        for s, _e, alias in spans:
            ...
            if bigram not in question:
                continue
            # 规则 1：前邻字+别名首字是功能词（"以临洲"=「以上」+「海」、"未达播"=「未达」+「播」）
            if bigram in _FUNCTION_BIGRAMS:
                spliced.append({"alias": alias, "bigram": bigram})
                continue
            # 规则 2：原文残段+书架节点拼成新串（"东北三省"→候选"东凛澜战区"=「东」+「凛澜战区」）。
            # 合法错字纠正除外：原文里有该别名的编辑距离≤1 变体（"看商泳事业群"→"看商用事业群"）。
