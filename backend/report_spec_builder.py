...
def _dataset_root_name(dataset: Dict[str, Any]) -> str:
    ...
    # 只要能匹配到“...事业群”，就取到事业群为止，避免残留“经营”等词
    match = re.search(r".*?事业群", name)
    if match:
...
def _is_dataset_root_name(name: str, dataset: Dict[str, Any]) -> bool:
    ...
    if not root or not name:
        return False
    # 保守策略：只过滤名称明确为“XX事业群”的根节点别名，避免误伤普通业务词
    if not root.endswith("事业群"):
        return False
    return name == root
...
def _requested_level_values(question: str, config: Dict[str, Any]) -> List[str]:
    ...
    common_values = ["城市战区", "城市站", "事业群", "行业部", "战区", "片区", "客户经理", "业务员", "部门", "线"]
    alias_map = {"城市站": "城市战区"}
...
# 末端个人节点（客户经理/个人）：这类节点才展示“个人业绩”，战区/城市战区/事业群一律不是末端
...
def _is_person_level(node: Optional[Dict[str, Any]]) -> bool:
    """只有客户经理/个人等人员层级才算末端个人节点；战区/城市战区/事业群不算。"""
    if not node:
        return False
    ...
    if re.search(r"(客户经理|业务员|销售员|个人)$", name):
        return True
    return False
...
def _infer_level_value_from_name(name: str, org_level: Any) -> str:
    text = str(name or "")
    if "城市战区" in text or "城市站" in text:
        return "城市战区"
    if "战区" in text:
        return "战区"
    if "事业群" in text:
        return "事业群"
    if re.search(r"(客户经理|业务员|销售员|个人)$", text):
        return "客户经理"
...
def _question_mentions_node(question: str, name: str) -> bool:
    ...
    if text and node_name:
        normalized_question = text.replace("城市战区", "城市站")
        normalized_node = node_name.replace("城市战区", "城市站")
...
def _resolved_member_names(resolved_entities: Optional[Dict[str, Any]]) -> List[str]:
    ...
    for name in names:
        ...
        alias = name.replace("城市分战区", "城市站").replace("城市战区", "城市站")
        if alias and alias not in expanded:
            expanded.append(alias)
        reverse_alias = name.replace("城市站", "城市战区")
...
def build_report_spec(
    ...
    # 只有题干明确出现对比/比较类词汇时才视为显式对比；避免“各战区排名”因错误解析出多个成员而被当成对比
    ...
    if mode == "comparative" and comparison_nodes:
        ...
        # 多行集合结果时 KPI 取合计（如电商 3 行行业部合计 14.96 亿），不是单节点值；
        ...
        if len(nodes) > 1 and len(level_values) <= 1:
            ...
            # 多根总览且结果集无父级汇总行（如"四大战区整体盘点"=4 战区+各自片区）：
    ...
    answer_mode = (
        ...
        # _level_overview（如"商用事业群整体业绩"）虽然 intent=ranking（SQL 生成复用 ranking 路径），
    ...
    if answer_mode == "ranking" and not answer_summary:
        ...
        # level_overview 场景（如"商用事业群整体业绩"）：显示层级结构总览，不是排名
        ...
        # 对比场景：answerSummary 取各对比节点的汇总行（如消费者/商用事业群），
        # 便于前端/摘要在对比双方维度直接展示任务、开单、达成率、缺口。
        # 显式对比且路由层已识别出被点名节点（如"消费者和商用的对比"）时，
        # sides 必须取 matched_nodes 本身（两个事业群总体行），
        # 而不是 focus 节点下级的 comparison_nodes[:2]（会错选成消费者下属战区）。
        side_source_nodes = (
            matched_nodes[:2]
