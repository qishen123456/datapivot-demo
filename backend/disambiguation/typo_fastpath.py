...
纯代码、毫秒级、不调 LLM。只拦"铁证级"错字：
  - 带组织后缀的片段（…战区/事业群/片区/行业部/城市站）不在别名表，
    但与某个别名编辑距离 ≤1 且候选唯一；
  - 人名（客户经理/承接人级别节点）：同长度子串编辑距离恰好为 1 且候选唯一。
...
安全约束（对应方案评审结论）：
  ...
  2. 泛层级词（"战区""片区"等单独出现）不参与匹配；
  3. 候选非唯一不拦；
  4. 别名最短 3 字——2 字串错 1 字=50% 差异，实测误拦"云集的业绩"→"沐阳"，
...
# 泛层级词单独出现时不参与错字匹配（"各战区的排名"里的"战区"不是实体）
GENERIC_LEVEL_WORDS = {
    "战区", "事业群", "片区", "行业部", "城市战区", "城市站",
    "客户经理", "业务员", "承接人", "业务承接人",
}

ORG_SUFFIXES = ("城市战区", "城市站", "战区", "事业群", "片区", "行业部", "客户经理")
# 组织片段：后缀 + 前面 2~6 个汉字
_ORG_FRAGMENT_RE = re.compile(
    r"[一-龥]{2,6}?(?:城市战区|城市站|战区|事业群|片区|行业部|客户经理)"
)

_PERSON_LEVELS = {"客户经理", "承接人", "业务承接人", "业务员", "员工", "个人"}

# 片段开头的连接词/虚词要剥掉（"澔原分公事和岚屿战区" 会切出 "和岚屿战区"）
...
def detect_obvious_typo(
    ...
    别名驱动滑窗匹配（覆盖后缀本身打错的场景，如"澔原分公事""云集直赢"）：
    ...
    try:
        ...
        for e in sorted(entries, key=lambda x: len(x["alias"]), reverse=True):
            alias = e["alias"]
            # 2 字别名不参与快检：错 1 字=50% 差异，实测会误拦"云集的业绩"→"沐阳"这类正常问法
            # （2 字人名纠错如"丁洁→程允文"主动放弃，交由后续守门员纠正条兜底，换取零误拦）
            if len(alias) < 3:
                continue
            ...
            for wlen in (alen, alen - 1, alen + 1):
                ...
                for start in range(0, qlen - wlen + 1):
                    ...
                    if window in alias_set:
                        continue
                    # 首字锚定：真实错字极少错在首字，且能挡住"金额超过"→"韩云舟"这类误配
                    if window[0] != alias[0]:
                        continue
