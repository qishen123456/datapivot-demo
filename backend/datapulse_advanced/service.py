...
class AdvancedAskService:
    ...
    def _cross_dataset_execution_question(question: str, route_guard: Dict[str, Any]) -> str:
        ...
        instruction = (
            "进阶跨数据集对比执行约束：本轮问题明确要求跨多个事业群/组织口径对比。"
            "请在每个命中的数据集内分别查询该数据集对应事业群的整体进度、达成率、总任务、实际完成和缺口；"
            "不要要求单个数据集同时包含另一个事业群名称；最终按各数据集结果并列对比。"
        )
        if mention_text:
    ...
    def _infer_subject_level(name: str) -> str:
        ...
        if "片区" in text:
            return "片区"
        if "客户经理" in text or "业务员" in text:
            return "客户经理"
        return "对象"
    ...
    def _build_subject_overview_from_rows(
        ...
        # 同一主体可能因为线/汇总视图出现多行，同名根节点优先按层级和空上级打分，
