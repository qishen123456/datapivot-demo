...
安全约束（对应方案评审结论）：
  ...
  3. 候选非唯一不拦；
  4. 别名最短 3 字——2 字串错 1 字=50% 差异，实测误拦"京东的业绩"→"碣澜"，
...
# 片段开头的连接词/虚词要剥掉（"沧澜分公事和淼澜战区" 会切出 "和淼澜战区"）
...
def detect_obvious_typo(
    ...
    别名驱动滑窗匹配（覆盖后缀本身打错的场景，如"沧澜分公事""京东直赢"）：
    ...
    try:
        ...
        for e in sorted(entries, key=lambda x: len(x["alias"]), reverse=True):
            alias = e["alias"]
            # 2 字别名不参与快检：错 1 字=50% 差异，实测会误拦"京东的业绩"→"碣澜"这类正常问法
