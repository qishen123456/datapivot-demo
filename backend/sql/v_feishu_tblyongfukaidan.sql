...
-- 修订内容：其他业务口径——"其它"/"网销商"的层级级别输出为"其他业务"而非"战区"，
-- 使报告统计战区=13 个真实战区（网销商/其它为用服的其他业务，不算战区，2026-09-06 用户拍板）。
-- 原始表 feishu_tblyongfukaidan 不动；战区字段的"用户服务与运营"后缀冗余由本视图 replace 剥离。
...
SELECT id,
  ...
  CASE WHEN fields ->> '战区'::text IN ('其它'::text, '网销商'::text)
       THEN '其他业务'::text
       ELSE fields ->> '层级级别'::text END AS "层级级别",
  fields ->> '事业群'::text AS "事业群",
  NULLIF(replace(fields ->> '战区'::text, '用户服务与运营'::text, ''::text), ''::text) AS "战区",
  fields ->> '链接字段(勿删)'::text AS "组织路径",
  NULLIF(TRIM(BOTH FROM fields ->> '年度开单金额(云芯部件+产品配件) （万）'::text), ''::text)::numeric AS "年度开单金额_云芯部件产品配件_万",
...
WHERE fields IS NOT NULL AND fields <> '{}'::jsonb
  ...
  AND COALESCE(fields ->> '战区'::text, '') NOT IN ('其它', '网销商');
