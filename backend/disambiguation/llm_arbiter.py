...
{{
  ...
  "options": [
    {{"option_id":"arbiter_dataset_1", "dataset_id":1, "label":"系统推荐：商用事业群", "scope_filter":{{}}, "reason":"为什么适合", "score": 85}}
  ],
  "candidate_scores": [
    {{"dataset_id": 1, "dataset_name": "商用事业群", "score": 85, "reason": "匹配理由"}}
  ],
  "auto_pick_option_id": "",
...
""".strip()
    ...
    def _fallback(self, question: str, candidates: List[Dict[str, Any]], candidate_contexts: List[CandidateContext]) -> Dict[str, Any]:
        if len(candidate_contexts) >= 2 and not _question_has_explicit_scope(question, candidate_contexts):
            ...
            forced["confirm_question"] = forced.get("confirm_question") or "问题中未明确指定事业群或数据集口径，请确认要分析哪个数据集："
            return forced
