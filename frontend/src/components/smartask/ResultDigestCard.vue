...
const normalizeLevelHint = (value) => {
  ...
  if (text.includes('事业群')) return '事业群'
  if (text.includes('客户经理') || text.includes('业务员')) return '客户经理'
  if (text.includes('片区')) return '片区'
  if (text.includes('城市站') || text.includes('城市战区')) return '城市战区'
  if (text.includes('战区')) return '战区'
  if (text.includes('行业部')) return '行业部'
  return ''
}
const explicitQuestionLevel = computed(() => {
  const text = questionText.value
  if (/事业群/.test(text)) return '事业群'
  if (/客户经理|业务员/.test(text)) return '客户经理'
  if (/片区/.test(text)) return '片区'
  if (/城市站|城市战区/.test(text)) return '城市战区'
  if (/行业部/.test(text)) return '行业部'
  if (/战区/.test(text)) return '战区'
  return ''
})
...
const normalizedRows = computed(() => rows.value.map((row) => {
  ...
  const nameKey = findColumn(row, [/节点名称/, /^name$/i, /名称/, /战区|片区|客户经理/])
...
// 问题里直接点名两个对象（如"苏观澜和钟自蹊的业绩"）也视为对比意图
...
const asksRepresentative = computed(() => /片区/.test(questionText.value))
const asksBusinessPerson = computed(() => /客户经理|业务员/.test(questionText.value))
const asksLowerNode = computed(() => asksRepresentative.value || asksBusinessPerson.value)
const lowerNodeLabel = computed(() => (asksBusinessPerson.value ? '客户经理' : '片区'))
...
const resolveFocusRow = (items = []) => {
  ...
  const managementRow = explicitRows.find(item => !/客户经理|业务员/.test(item.level))
...
const managementLayerRows = computed(() => {
  ...
  const levelOrder = ['战区', '行业部', '片区', '城市站']
...
const comparisonRows = computed(() => {
  ...
  const officeRows = normalizedRows.value.filter((item) => {
    ...
    return /事业群|战区|行业部|片区|数据集/.test(item.level) || /事业群|战区|行业部|片区/.test(item.name)
  })
...
const comparisonLevelLabel = computed(() => {
  ...
  if (name.includes('事业群')) return '事业群'
  if (name.includes('行业部')) return '行业部'
  if (name.includes('战区')) return '战区'
  if (name.includes('片区')) return '片区'
  return '对象'
})
...
// 否则会出现"战区 1 个、城市战区 6 个"这种把聚焦节点当数据点的错觉。
...
const expectedDetailLevel = computed(() => {
  ...
  if (digestCompareLevel.value === '战区') {
    return normalizedRows.value.some(item => item.level === '片区') ? '片区' : '城市站'
  }
  if (digestCompareLevel.value === '行业部' || digestCompareLevel.value === '片区') return '客户经理'
  return ''
})
...
const secondaryDrillRows = computed(() => {
  ...
  // filter 问题（末层命中，如"达成率小于10%的城市战区"）：结果分布条形图
  // （showFilterRateChart）已完整呈现命中节点，且城市战区已是最后一层、无可再下钻，
...
const secondaryDrillGroups = computed(() => {
  ...
  return orderedParents
    ...
    .filter(group => group.rows.length)
    // 多层级混合结果（如 29 行盘点：战区+片区）按层级排序，上级分组在前
...
// 二级拆解分组的层级排序与标签（bug 2026-08-22：四大战区盘点 29 行场景下，
// 战区分组被排到片区分组之后，且分组标签误用全局 detailLevelLabel 显示成"4个片区"）
const DRILL_GROUP_LEVEL_ORDER = { 事业群: 0, 战区: 1, 行业部: 1, 城市战区: 1, 城市站: 1, 片区: 2, 客户经理: 3, 业务员: 3, 承接人: 3 }
...
const isBusinessPersonRanking = computed(() => (
  isRankingQuestion.value && secondaryLevelLabel.value === '客户经理'
))
...
const secondaryRateTone = (row) => {
  ...
  // - 中性排名（如“战区业绩排名”）按相对名次分层：前1/3绿、中1/3黄、后1/3红
  if (isRankingQuestion.value) {
    if (rankSides.value === 'both') {
