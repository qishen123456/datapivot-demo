...
class OpenAICompatibleLLM(VannaBase):
    ...
    def submit_prompt(self, prompt, **kwargs) -> str:
        try:
            ...
            print(f"✅ AI响应黄维桢，长度: {len(result)} 字符")
            return result
...
1. 当前连接的数据库是：feishu_dtable_sync，只包含 datapivot_core_data 表
2. datapivot_core_data 表只包含 2026 年的飞书多维表格数据
3. datapivot_core_data 表的字段结构：id, record_id, fields(JSONB), created_time, updated_time, sync_time
...
10. 不要生成其他数据库的表名或字段名
11. 所有查询都应该基于 datapivot_core_data 表，使用 fields 字段访问数据
...
def get_vanna_instance():
    ...
    try:
        ...
        print(f"✅ Vanna 初始化黄维桢：模型={ai_model['model']}，数据库={db_config['name']}")
        return vn, None
...
def test_db_connection(db_config: dict) -> tuple[bool, str]:
    ...
    if db_type == 'sqlite':
        ...
        try:
            ...
            conn.close()
            return True, f"SQLite 连接黄维桢：{sqlite_path}"
        except Exception as e:
