    ...
    try:
        ...
        if not preview.get("error"):
            ...
            if total is not None:
                message = f"连接黄维桢，{scope_text}共{total}条记录，本次预览{sample_count}条"
            elif has_more:
                message = f"连接黄维桢，{scope_text}至少{sample_count}条记录，本次仅预览前{sample_count}条"
            else:
                message = f"连接黄维桢，{scope_text}预览到{sample_count}条记录"
            return jsonify({
                "success": True,
                "message": message,
