...
def _build_display_title(question: str, dataset_result: Dict[str, Any]) -> str:
    ...
    if intent == "ranking":
        ...
        if top_n == 1 and (
            ...
            return title
        # 排名类标题：数据集 + 层级 + 指标 + 排名（+ TopN），避免“达成率的战区”这种倒装
        if target_level:
