"""
Import datapivot_core_data rows from a portable JSON bundle.

Usage:
  python import_datapivot_core_data.py /app/backend/imports/datapivot_core_data_bundle.json
"""
...
def _reset_sequence(cur):
    cur.execute(
        """
        SELECT pg_get_serial_sequence('datapivot_core_data', 'id') AS seq_name;
        """
    )
    row = cur.fetchone() or {}
    ...
    if not seq_name:
        return
    cur.execute("SELECT COALESCE(MAX(id), 0) AS max_id FROM datapivot_core_data;")
...
def import_bundle(bundle_path: str) -> Dict[str, Any]:
    ...
    with repo._connect() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
        _ensure_table(cur)
        cur.execute("TRUNCATE TABLE datapivot_core_data RESTART IDENTITY;")

        for row in rows:
            cur.execute(
                """
                INSERT INTO datapivot_core_data (id, record_id, fields, sync_time, created_time, updated_time)
                VALUES (%s, %s, %s, %s, %s, %s);
                """,
                (
                    row.get("id"),
...
def main():
    if len(sys.argv) < 2:
        raise SystemExit("Usage: python import_datapivot_core_data.py /path/to/datapivot_core_data_bundle.json")
