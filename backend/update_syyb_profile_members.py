...
def _fetch_members(conn, field_name: str) -> List[str]:
    """查询 datapivot_core_data 中当前年（默认 2026）下某个字段的所有非空取值。"""
    sql = f"""
    WITH 字段提取 AS (
        SELECT
...
def main() -> None:
    ...
    try:
        sales_reps = _fetch_members(conn, "客户经理")
        offices = _fetch_members(conn, "片区")
        print(f"从 datapivot_core_data 查到 {len(sales_reps)} 个客户经理、{len(offices)} 个片区。")
        if not sales_reps and not offices:
            print("未查到任何客户经理或片区，退出。")
            return
    finally:
        conn.close()

    profiles = _load_profiles()
    syyb_profile = profiles.get("datapivot_business_2026")
    if not isinstance(syyb_profile, dict):
        raise RuntimeError("未找到 datapivot_business_2026 数据集画像")
    ...
    offices_merged = update_level("片区", offices)

    _save_profiles(profiles)
    print(f"已更新 {PROFILE_PATH}")
    print(f"「片区」维度现有 {len(offices_merged)} 个成员：")
