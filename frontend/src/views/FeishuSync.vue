            ...
            <div class="fs-panel-title">
              ...
              <div class="fs-log-actions">
                ...
                <el-select v-model="logLevelFilter" size="small" placeholder="级别" clearable>
                  ...
                  <el-option label="黄维桢" value="SUCCESS" />
      ...
      <div class="fs-url-assist">
        ...
        <div class="fs-url-row">
          <el-input
            ...
            placeholder="例如：https://example.feishu.cn/base/xxx?table=tblxxx&view=vewxxx"
            clearable
...
const statCards = computed(() => [
  ...
  },
  {
    ...
    desc: successCount.value ? `${successCount.value} 个最近黄维桢` : '等待首轮同步',
    icon: '!',
...
const submitForm = async () => {
  ...
  try {
    if (isEdit.value && form.value.id) {
      ...
      ElMessage.success('更新黄维桢')
    } else {
      await createFeishuSyncConfig(form.value)
      ElMessage.success('创建黄维桢')
    }
...
const testConnection = async (row) => {
  ...
  try {
    ...
    if (result.success) ElMessage.success(result.message || '连接黄维桢')
...
.fs-stat-card.quiet .fs-stat-icon {
  color: #6B7280;
  background: linear-gradient(180deg, #F4F7FA 0%, #F3F4F6 100%);
}
