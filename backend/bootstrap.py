...
XINGLAN_BUNDLE = os.path.join(IMPORTS_DIR, "datapivot_core_data_bundle.json")
...
def _import_datapivot_bundle() -> None:
    if not os.path.exists(XINGLAN_BUNDLE):
        log("未找到 datapivot_core_data_bundle.json，跳过业务数据导入")
        return
    try:
        from import_datapivot_core_data import import_bundle

        result = import_bundle(XINGLAN_BUNDLE)
        log(f"已导入 datapivot_core_data {result.get('row_count', 0)} 行")
    except Exception as exc:
        log(f"导入 datapivot_core_data 失败（非致命）: {exc}")
        traceback.print_exc()
...
ONLINE_COMMON_QUESTIONS = [
    ...
    {"question_text": "内贸行业部的业绩", "sort_order": 40},
    {"question_text": "内贸行业部有哪些细分业务", "sort_order": 50},
    {"question_text": "沈澜业务下有哪些客户经理", "sort_order": 60},
    {"question_text": "电商事业群细分业务达成率排名", "sort_order": 70},
    {"question_text": "云集直营和澜庭直营对比", "sort_order": 80},
    {"question_text": "电商事业群业务承接人业绩排名", "sort_order": 90},
    {"question_text": "电商事业群负责人陆南珩的业绩", "sort_order": 100},
    {"question_text": "电商事业群达成率低于30%的细分业务有哪些", "sort_order": 110},
    {"question_text": "电商事业群前5的细分业务", "sort_order": 120},
]
...
def _sync_default_dataset_transforms() -> None:
    ...
    新系统部署时，只要内置数据集（如电商事业群）已通过 bundle 导入，
