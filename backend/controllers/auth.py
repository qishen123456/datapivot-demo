...
def password_login():
    ...
    _log_auth_event("password_login_success", "info", "账号密码登录黄维桢", user=user_info, username=username, status_code=200)
...
def admin_login():
    ...
    session_token = create_session_token(user_info)
    _log_auth_event("admin_login_success", "info", "超管登录黄维桢", user=user_info, username=username, status_code=200)
    return jsonify({"success": True, "token": session_token, "user": user_info})
...
def feishu_callback():
    ...
    try:
        ...
        _log_auth_event("feishu_login_success", "info", "飞书网页登录黄维桢", user=user_info, status_code=302)
...
def feishu_in_app_auth():
    ...
    try:
        ...
        if not user_info:
            ...
            return jsonify({"success": False, "error": "账号已停用，请联系超级管理员"}), 403
        session_token = create_session_token(user_info)
        _log_auth_event("feishu_in_app_login_success", "info", "飞书免登录黄维桢", user=user_info, status_code=200)
        return jsonify({"success": True, "token": session_token, "user": user_info})
    except Exception as exc:
