  ...
  const buckets = [
    {
      ...
      lines: [
        ...
        '正在组织只读 PostgreSQL 查询，重点校准战区、片区、时间范围和指标口径。',
    ...
    },
    {
      ...
      lines: [
        ...
        '正在排查可能的层级口径风险，例如组织简称、片区归属和金额聚合方向。',
...
const EXPLICIT_DATASET_SCOPE_RE = /消费者(?:事业群)?|商用(?:事业群)?|电商(?:事业群)?/
const ORG_LEVEL_TERMS = ['战区', '城市战区', '城市站', '行业部', '片区']
// 个人层词：消费者数据集只有机构层（事业群/战区/城市战区），
// “客户经理/业务员”这类个人层仅存在于商用数据集。
// 追问点名个人层时，目标数据集可能已切换到商用，不能再沿用上一轮确认的数据集。
const PERSON_LEVEL_TERMS = ['客户经理', '业务员']
...
const buildChartPreview = (dataset) => {
  ...
  const categoryColumn =
    columns.find(col => /日期|时间|城市|战区|名称|类型|水平/i.test(col)) ||
