...
class IntentResolver:
    ...
    def resolve(self, question: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 根层级通常是 analysisDimensions 中每条 path 的第一个节点（如 电商事业群/消费者事业群）。
        ...
        def resolve_target_level_from_text() -> str:
            ...
            if "城市战区" in text and "城市战区" not in {m["candidate"] for m in matches}:
                matches.append({"candidate": "城市战区", "level": "城市战区", "pos": text.rfind("城市战区"), "is_root": False})
            for dimension in config.get("analysisDimensions") or []:
        ...
        level_only_filter = bool(
            re.search(r'''属于['""“”](?:行业部|片区|战区|城市战区|客户经理|城市站|区域线|行业线)['""“”](?:层级|层|节点|数据)?''', text)
            or re.search(r"(?:哪些|哪个|哪家|哪几个|找出|筛选出).*(?:区域线|行业线)", text)
        )
        ...
        threshold_filter_question = bool(
            ...
            and re.search(r"(?:城市战区|城市站|战区|片区|行业部|业务员|客户经理|客户经理|细分业务)", text)
        )
        ...
        list_children_question = bool(
            ...
            and re.search(r"(?:客户经理|负责人|细分业务|业务线|行业部)", text)
        )
        ...
        if asks_aggregate and not re.search(r"\d+(?:\.\d+)?\s*%?", text):
            ...
            level_like_values = {"事业群", "战区", "行业部", "片区", "客户经理", "城市战区", "城市站", "区域线", "行业线"}
            org_suffixes = ["战区", "片区", "行业部", "事业群", "城市战区", "城市站", "客户经理"]
        ...
        level_like_values = {"事业群", "战区", "行业部", "片区", "客户经理", "城市战区", "城市站", "区域线", "行业线"}
        org_suffixes = ["战区", "片区", "行业部", "事业群", "城市战区", "城市站"]
        resolved_names = self.ports.resolved_entity_names(context)
        ...
        # 没命中层级词，但提取到多个看起来像人名的对象时，兜底到客户经理层级
        if (
            not target_level
        ...
        # 过滤掉数据集根节点别名，避免把“商用事业群”本身也当成查询对象
        dataset = self.ports.safe_dict(context.get("dataset"))
        ...
        if (
            ...
            person_levels = {"客户经理", "业务员", "承接人", "负责人", "个人"}
        ...
        if (
            ...
            for name in resolved_names:
                if not name or name in level_like_values:
                    continue
                # 节点名本身已经是目标层级实例的（如"客户经理钟自蹊"中的"钟自蹊"若 endswith 客户经理），不应视为下钻
        ...
        # 如果整体满足 filter 条件（如“看下大于一个亿的战区”），优先走 filter 逻辑，不要误判为对比
        if filter_problem:
            symbol_match = None
        ...
        # "没有业绩的客户经理有哪些"无法命中本分支 → 被 ranking/drilldown 抽签劫持）
        ...
        if filter_problem:
            ...
            # 纯层级/线过滤，没有附带数值阈值，不要把题干里的数字当成阈值
        ...
        if not matched_triggers:
            ...
            if is_level_overview:
                ...
                if resolved_names:
                    ...
                    # 根节点（事业群）问题：仅当用户明确带"整体/总体/总览/汇总/全部"时才走整体概览；
                    # 不带这些词（如"电商事业群的业绩"）应走 §1.4 根节点默认带下级，不能被 level_overview 拦截。
                    overview_markers = ("整体", "总体", "总览", "汇总", "全部", "全局")
                    has_explicit_overview = any(kw in text for kw in overview_markers)
                    # 关键修复：根节点别名（消费者事业群/商用事业群/电商事业群）作为 target_level 时，
                    # 即使没"整体"词也走 level_overview 路径，避免落到 entity scope_filter 全下级瀑布。
                    is_root_level_target = target_level in {"消费者事业群", "商用事业群", "电商事业群"}
                    if any(
        ...
        # 单点最高/最低问法（“哪个最高/最低”或“最低的战区”）默认只取 1 个，避免和“排名前 N”混淆
