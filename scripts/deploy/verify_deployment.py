...
校验项：
- PostgreSQL 可连接
- 关键 schema 表存在 (bs_datasets, datapivot_core_data 等)
- bs_datasets 已导入数据
- 报告场景识别与报告模板契约校验模块可用
- Flask /api/health 健康
...
用法：
    docker compose run --rm backend python /app/scripts/deploy/verify_deployment.py
    或
...
def _check_report_contract() -> List[Tuple[str, bool, str]]:
    ...
    sample_columns = ["节点名称", "上级名称", "层级", "线", "任务金额", "开单金额", "达成率"]
    try:
