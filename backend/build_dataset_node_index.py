...
def _consumer_nodes_sql() -> str:
    ...
    division_expr = _extract_jsonb_text("事业群")
    branch_expr = _extract_jsonb_text("战区")
    city_expr = _extract_jsonb_text("城市战区")
    return f"""
WITH base AS (
    SELECT
...
nodes AS (
    SELECT DISTINCT
        '事业群' AS node_level,
        COALESCE(NULLIF(事业群, ''), '消费者事业群') AS node_name,
    ...
    SELECT DISTINCT
        '战区' AS node_level,
        战区 AS node_name,
        COALESCE(NULLIF(事业群, ''), '消费者事业群') AS parent_name,
        '消费者经营链路' AS track
    FROM base
    WHERE 战区 <> ''
    UNION ALL
...
WITH nodes AS (
    SELECT DISTINCT
        '事业群' AS node_level,
        COALESCE(NULLIF(TRIM(事业群), ''), '用户服务与运营事业群') AS node_name,
        NULL::TEXT AS parent_name,
        '用服经营链路' AS track
    ...
    SELECT DISTINCT
        '战区' AS node_level,
        TRIM(战区) AS node_name,
        COALESCE(NULLIF(TRIM(事业群), ''), '用户服务与运营事业群') AS parent_name,
        '用服经营链路' AS track
    FROM v_feishu_tblyongfukaidan
    WHERE 层级级别 = '战区' AND COALESCE(TRIM(战区), '') <> ''
)
SELECT node_level, node_name, parent_name, track
FROM (
    SELECT DISTINCT node_level, node_name, parent_name, track
