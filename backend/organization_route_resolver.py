...
ORG_SUFFIXES = ("城市战区", "城市站", "战区", "事业群", "行业部", "片区")
...
def _alias_matches_question(alias: str, node_name: str, normalized_question: str) -> bool:
    ...
    # 数据集域前缀别名豁免（2026-09-03 用服接入实测）：事业群根节点的"额外别名"
    # （非节点名前缀，如"用服"之于"用户服务与运营事业群"）紧跟层级词时是
    # "域+层级"用法（"用服战区"=用服事业群的战区），不是后缀冲突。
    # 若按普通冲突拒绝，"用服战区云芯部件排名"的"用服"会被丢弃，路由被其他数据集的
    # 节点别名（如电商"云芯部件"）拐走。只豁免根节点额外别名，后缀剥离别名（"消费者"）
    # 保持原冲突判定不变。
    is_root_scope_alias = bool(alias) and not compact_name.startswith(alias) and node_suffix == "事业群"
    start = 0
    while True:
...
class OrganizationRouteResolver:
    ...
    # bug 2026-08-26：泛称+层级分解。用户问"商用战区的业绩"时，
    # "商用战区"不是真实节点名（实际节点是澔原/瀚川/岚屿/朔岑战区），
    # 旧逻辑把它当节点名塞进 节点名称 IN (...) → 必然 0 行。
    # 分解规则：entity = <节点名或别名> + <层级词>，如 "商用"+"战区" →
    # 过滤条件应为 层级='战区' AND 上级名称='商用事业群'（返回该层级全部子节点）。
    _GENERIC_LEVEL_WORDS = ("城市战区", "战区", "行业部", "事业群", "片区", "城市站")

    def resolve_generic_level_entity(self, entity: str, dataset_id: int) -> Optional[Dict[str, str]]:
        """'商用战区' → {'level': '战区', 'parent_name': '商用事业群'}。
    ...
    def resolve(
        ...
        # 数据集域前缀收窄（2026-09-03 用服接入实测）：根节点（事业群级）别名与其他
        # 数据集的节点同时命中、且根别名只属于一个数据集时，根别名表达的是"选数据集"
        # 而不是"选节点"（"用服沧岩"=用服事业群的沧岩战区）。把 mentions 收窄到
        # 该数据集并优先保留非根节点，避免"用服沧岩战区的业绩"被弹成跨数据集确认卡。
        # 单数据集场景不动（"商用事业群和澔原战区对比"的根节点仍是有效成员）。
        root_mentions = [m for m in mentions if _compact(m.get("node_name")).endswith("事业群")]
        ...
        if len(dataset_ids) > 1 and len(root_dataset_ids) == 1:
            ...
            finer = [m for m in narrowed if not _compact(m.get("node_name")).endswith("事业群")]
    ...
    def domain_prefix_scope_id(
        ...
        用服接入实测（2026-09-03）：LLM 主体纠正会把"用服沧岩"的域前缀"用服"
        当口语剥掉（改写为"沧岩战区的业绩"），路由只剩跨数据集同名节点，
        在消费者/用服之间误弹确认卡。以下条件全部满足时，调用方应放弃改写、
        保留原始问题进入路由（resolve 的域前缀收窄会锁定该数据集）：
        1. 原始问题命中"唯一根节点域别名 + 跨数据集节点"（同 resolve 收窄条件）；
        2. 改写后该条件不再成立（前缀被剥掉）；
        3. 改写保留的主体仍是真实组织节点——防止"商用事业群程允文"→"程允文"这类
        ...
        def _scope_id(text: str) -> Optional[int]:
            ...
            root_dataset_ids = _unique([
                ds
                for m in mentions
                if _compact(m.get("node_name")).endswith("事业群")
