...
class RouteGuardSkill:
    ...
    def _looks_like_branch_dataset_ambiguity(question: str, candidates: List[Dict[str, Any]]) -> bool:
        ...
        if not compact or "战区" not in compact:
            return False
        if any(token in compact for token in ["商用", "商用事业群", "消费者", "消费者事业群", "城市战区", "城市站", "片区", "行业部", "业务员", "客户经理"]):
            return False
        names = [str(item.get("dataset_name") or "") for item in (candidates or [])[:3]]
        has_commercial = any("商用事业群" in name for name in names)
                ...
                result.update(
                    {
                        ...
                        "reason": "问题明确要求多个事业群/组织口径对比，已覆盖当前手动选择并纳入全部命中数据集。",
                    }
                )
                ...
                result.update(
                    {
                        ...
                        "apply_dataset_ids": org_dataset_ids,
                        "recommended_dataset_ids": org_dataset_ids,
                        "reason": "问题明确要求多个事业群/组织口径对比，且组织树命中多个数据集，进阶流程按跨数据集对比执行。",
                    }
                )
                return result
            ...
            result.update(
                {
                    "action": "needs_confirmation",
                    "confidence": "medium",
                    "reason": "问题只提到“战区”，但当前候选同时包含商用事业群和消费者事业群，存在数据集歧义，需要先确认口径。",
                }
            )
            result["warnings"].append("请先确认是商用事业群战区，还是消费者事业群战区。")
            return result
