...
class FourAgentAskService:
    ...
    def _auto_build_detail_table(self, report_spec: Optional[Dict[str, Any]], rows: List[Dict[str, Any]]) -> str:
        ...
        targetLevel=战区 或 rows 含层级=战区的行（兼容 LLM 生成 report_spec 时 targetLevel 缺失的场景）。
        ...
        if answer_mode and answer_mode not in ("comparative", "detail"):
            return ""
        # 层级判断：targetLevel=战区 或 rows 含层级=战区的行
        has_branch_level = target_level == "战区" or any(
            str(r.get("层级") or "") == "战区" for r in rows
        )
        ...
        # 数据自带合计行（层级=事业群，如 2026-09-06 golden 2696 UNION 的事业群权威总计）：
        # 不参与明细排序与求和（否则合计=明细+总计翻倍），固定加粗渲染为末行。
        builtin_totals = [r for r in rows if str(r.get("层级") or "") == "事业群"]
        detail_rows = [r for r in rows if str(r.get("层级") or "") != "事业群"]
        try:
    ...
    def _build_layered_management_report(
        ...
        track_col = str(config.get("trackColumn") or "线")
        ...
        if explicit_person_focus and focus_person_row:
            ...
            lines = [
                ...
                f"• **归属链路：** {parent_name or '未识别上级'} -> 可继续下钻查看所属行业部/片区的整体支撑情况。",
                "",
                "### 问题诊断",
        ...
        if is_comparison and len(top_section["rows"]) >= 2:
            ...
            if len(compared) >= 2:
                ...
                lines = [
                    ...
                    f"🔴 **共同改进项**：先针对低达成{child_level_label}建立周度跟进清单，再下钻到客户经理按缺口金额和项目阶段排序。",
                ]
                if error_message:
                    lines.extend(["", f"> 说明：高级模型分析失败，已使用规则分层报告兜底。原因：{error_message}"])
                return "\n".join(lines)
    ...
    def _build_route_confidence(self, route: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if reason in {"explicit_dataset_scope_unique", "explicit_dataset_domain", "explicit_dataset_alias"}:
            score += 4
        elif reason in {"organization_tree_name_resolved", "entity_mention_unique", "target_level_unique:城市战区"}:
            score += 3
    ...
    def _select_sql_strategy(self, question: str, route: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        requested_city_level = (
            str(query_intent.get("target_level") or "") == "城市战区"
            or "城市战区" in level_hint_text
        )
        # 注意：filter/comparison/aggregate 判定不能在构建规则 SQL 之前固化，
        # _build_rule_based_sql 的数量词修复会就地改写 query_intent["intent"]（如"四大战区"→aggregate），
        # 提前捕获会让守卫失效、被 Golden 样本劫持（bug 2026-08-21 四大战区整体盘点 101 行瀑布）。
        ...
        def question_key(value: Any) -> str:
            ...
            # 助词"的"不影响问法同一性（"用服事业群的业绩"≡"用服事业群业绩"），
            # 不剥会让主体纠正改写（LLM 爱加"的"）精确错失 golden 直通（2026-09-03 实测）
            text = text.replace("的", "")
            text = text.replace("消费者事业群", "").replace("消费事业群", "").replace("消费者", "")
            text = text.replace("商用事业群", "").replace("商用", "")
            return text
        ...
        def sample_conflicts_requested_level(sql: str) -> bool:
            ...
            asks_branch_level = "层级='战区'" in compact_sql or '层级="战区"' in compact_sql
            asks_city_level = "层级='城市战区'" in compact_sql or '层级="城市战区"' in compact_sql
            return asks_branch_level and not asks_city_level

        # bug#1 Golden 样本层级词 LIKE 劫持守卫：样本 SQL 用 LIKE '%层级词%' 匹配层级，
        # 但问题点名的是具体节点（如"衡原片区"）且节点名不含该层级词，则不采样本。
        # 反例：样本 LIKE '%片区%' 命中"衡原片区业绩"，会拉来 95 行所有片区，应弃样本走规则 SQL。
        _LEVEL_LIKE_PATTERN = re.compile(
            r"(?:节点名称|上级名称|战区|片区|行业部)\s+LIKE\s+'%([^%]+)%'",
            re.IGNORECASE,
        )
    ...
    def _coerce_city_company_level_sql(self, question: Any, context: Dict[str, Any], sql_text: str) -> str:
        ...
        if not is_consumer_dataset:
            return sql
        sql = re.sub(
            r"(?P<col>\"?层级\"?)\s*=\s*(?P<quote>['\"])战区(?P=quote)",
            lambda match: f"{match.group('col')} = {match.group('quote')}城市战区{match.group('quote')}",
            sql,
        )
        return re.sub(
            r"(?P<col>\"?层级\"?)\s+IN\s*\(\s*(?P<quote>['\"])战区(?P=quote)\s*\)",
            lambda match: f"{match.group('col')} IN ({match.group('quote')}城市战区{match.group('quote')})",
            sql,
            flags=re.I,
        )
    ...
    def _sql_subject_filter_literals(cls, sql_text: str) -> List[str]:
        ...
        subject_cols = r"(?:节点名称|上级名称|客户经理|战区|片区|行业部)"
        for match in re.finditer(condition_prefix + rf"[^;\n]{{0,100}}?\b{subject_cols}\b\s*=\s*'([^']+)'", sql, flags=re.I):
            value = str(match.group(1) or "").strip()
            if value and value not in values and value not in {"商用事业群", "消费者事业群"}:
                values.append(value)
    ...
    def _node_index_subject_names_from_question(self, question: str, context: Dict[str, Any]) -> List[str]:
        ...
        generic_level_aliases = {
            "事业群", "战区", "片区", "行业部", "城市战区", "城市站",
            "客户经理", "业务员", "承接人", "业务承接人", "业务承接角色",
        }
        expanded_text = text
        compound_aliases = {
            "东漠澜": "沧澜 漠澜",
        ...
        for alias_item in node_index.get("flat_alias_index") or []:
            ...
            for match in alias_item.get("matches") or []:
                ...
                if not node_name or node_name in generic_level_aliases or node_level in {"事业群"}:
                    continue
    ...
    def _question_subject_names(self, question: str, context: Dict[str, Any], include_resolved: bool = True) -> List[str]:
        ...
        # 优先提取带角色前缀的具体人名（如“客户经理钟自蹊”），角色前缀比语义解析更精确，
        # 避免把“商用事业群”这样的数据集根节点别名误判为查询主体。
        ...
        level_only_values = {"战区", "片区", "行业部", "事业群", "城市站", "城市战区", "客户经理"}
        ...
        for match in re.findall(r"[\u4e00-\u9fa5A-Za-z0-9（）()]+?(?:片区|战区|行业部|事业群)", text):
        ...
        # 支持并列人名："苏观澜和钟自蹊的业绩"、"苏观澜、钟自蹊及钱明的业绩"
        ...
        person_patterns = [
            ...
            r"(?:问的是|查询的是|看的是|主体是|节点是|人员是|业务员是|客户经理是)\s*(" + coordinated_name + r")",
            r"(?<![\u4e00-\u9fa5])(" + coordinated_name + r")(?:的)?" + metric_person_suffix,
        ]
        non_person_terms = {
            "客户经理", "业务员", "客户经理", "销售人员", "销售", "片区",
            "战区", "行业部", "事业群", "城市站", "城市战区", "商用事业群",
        }
        for pattern in person_patterns:
            for match in re.findall(pattern, text):
                ...
                # 正则贪婪可能把"的"吞入人名，如"钟自蹊的总任务金额" -> "钟自蹊的总"
        ...
        org_pattern = (
            ...
            r"\s*([\u4e00-\u9fa5A-Za-z0-9（）()]+?(?:片区|战区|行业部|城市战区|城市站))"
    ...
    def _generic_level_dataset_matches(
        ...
        """
        当候选词是纯粹的通用层级词（如"战区"）时，按数据集聚合并返回每个支持该层级的数据集代表。
        这样可以让"战区业绩如何"这类问题弹数据集确认，而不是被 LLM 擅自选走。
        """
    ...
    def _role_person_subject_names(self, question: str) -> List[str]:
        ...
        role_person_patterns = [
            r"(?:商用事业群|商用|星澜商用事业群)?(?:的)?(?:客户经理|业务员|客户经理|销售人员|销售)\s*([\u4e00-\u9fa5]{2,4}(?:(?:和|与|及|跟|、|,|，)[\u4e00-\u9fa5]{2,4})+|[\u4e00-\u9fa5]{2,4})(?=\s*(?:的|业绩|绩效|达成率|达成|开单|完成情况|完成|情况|表现|总任务金额|任务金额|年度开单金额|开单金额|剩余任务金额|剩余金额|缺口|金额|总数|总量|总额|数量|$|[，,。？?]))",
            r"(?:客户经理|业务员|客户经理|销售人员|销售)(?:是|为|叫|：|:)\s*([\u4e00-\u9fa5]{2,4})",
        ]
...
"""
    ...
    def _is_pure_generic_level_question(self, question: str, matched_levels: List[str]) -> bool:
        ...
        没有任何明确的事业群/数据集别名标识。此类问题在多数据集都支持该层级时应弹确认。
        """
    ...
    def _question_target_level_hint(cls, question: str) -> str:
        ...
        if not text:
            return ""
        for level in ("城市战区", "城市站", "客户经理", "客户经理", "业务员", "片区", "行业部", "战区", "事业群"):
    ...
    def _map_dimension_aliases_for_dataset(cls, question: str, dataset: Dict[str, Any]) -> str:
        ...
        # 优先匹配「客户经理/业务员/销售」这类一线角色，再匹配「片区」等组织层级
        target_alias = ""
        role_priority = ("客户经理", "业务员", "销售")
        ...
        if not target_alias:
            for level in person_levels:
                aliases = [str(a).strip() for a in (level.get("aliases") or []) if str(a).strip()]
                preferred = next((a for a in aliases if "客户经理" in a or "业务员" in a), "")
                if not preferred:
        ...
        if "客户经理" in replaced:
            replaced = replaced.replace("客户经理", target_alias)
    ...
    def _dataset_alias_match_score(
        ...
        generic_aliases = {
            ...
            "战区", "片区", "行业部", "城市站", "城市战区", "业务员", "客户经理", "事业群",
        }
        ...
        generic_business_terms = {
            "分", "公司", "战区", "代表", "片区", "业务", "行业部", "城市", "城市站", "城市战区", "事业群",
        }
        score = 0
        for alias in alias_candidates:
            ...
            for match in re.findall(r"([\u4e00-\u9fff]{2,}?)(?:事业群|战区|片区|行业部)", normalized_alias):
                if len(match) >= 2:
    ...
    def _dataset_scope_alias_score(self, question: str, dataset: Dict[str, Any]) -> int:
        ...
        for alias_type, alias in alias_candidates:
            ...
            if normalized_alias in generic_aliases or normalized_alias in metric_only_aliases:
                continue
            # bug#11：synonym 为"商用事业群"时，问题"看下前三的商用战区"中的"商用"
            # 无法命中完整别名。对以"事业群"结尾的 synonym 增加前缀匹配：
            # 尝试匹配去掉"事业群"的前缀（如"商用"），但前缀长度需 >= 2 且不在 generic_aliases 中。
            matched = normalized_alias in normalized_question
            prefix_matched = False
            if not matched and normalized_alias.endswith("事业群"):
    ...
    def _compute_dataset_match(self, question: str, dataset: Dict[str, Any], context: Dict[str, Any]) -> int:
        ...
        schema_hit = 1 if any(token in dataset_tokens for token in ("日期", "时间", "金额", "战区", "事业群", "片区", "客户经理", "行业部", "城市战区", "区域")) else 0
        ...
        if self._looks_like_person_performance_question(question) and any(
            token in dataset_text for token in ("客户经理", "业务员", "销售人员")
        ):
            score = max(score, 88)
    ...
    def _extract_supported_levels(cls, dataset: Dict[str, Any], context: Dict[str, Any]) -> List[str]:
        ordered_levels = ["事业群", "战区", "城市战区", "城市站", "片区", "行业部", "客户经理", "业务员"]
        seen: List[str] = []
    ...
    def _detect_ambiguity(self, question: str, ranked_candidates: List[Tuple[Dict[str, Any], int]]) -> Optional[Dict[str, Any]]:
        ...
        # 分数接近时，按真实数据集名称确认，不再使用无意义的“战区层级”文案
        org_ambiguity_terms = ["战区", "城市战区", "城市站", "片区", "行业部", "客户经理", "业务员", "线", "区域", "团队", "组织"]
...
3. 【强制确认规则】如果用户问题中没有明确出现事业群名称（如"电商事业群"、"商用事业群"、"消费者事业群"）或数据集名称/别名，且存在 2 个及以上候选数据集都可能回答该问题，则必须 requires_confirmation=true，不得擅自选择。
...
"""
    ...
    def route_with_agent1(
        ...
        # 修复"查询程允文"等场景：这些问题的 _looks_like_org_subject_question 为 False，
        # 会走到 LLM arbiter 打分，但 LLM 不知道程允文具体在哪个数据集，结果"三个都可能包含"。
        ...
        if self._looks_like_org_subject_question(question):
            ...
            if subject_name:
                ...
                # 让下游返回 0 行 + "未找到"提示，避免被当成"战区"泛称口径弹确认或静默降级总览。
                if (
        ...
        for dataset in catalog:
            # 只使用数据集/业务域级别的显式标识，避免“行业部”等通用层级把无关数据集拉进来
        ...
        # 总分相同时，别名/业务域命中分高的优先，避免“电商事业群年度开单”被同义词“年度开单”顶到前面
        ...
        try:
            ...
            initials_hint = ""
            if not raw_hits:
                # 拉丁缩写扩展：sh → 临洲/深圳（首字母护栏同款映射，人工精排表优先）
                try:
            ...
            # 最长别名优先：题干已被长别名精确到节点时（"临洲城市站"），
            # 丢弃其短前缀（"临洲"）带出的兄弟节点，避免过度确认
            if raw_hits:
        ...
        exclusive_level_hints = {"城市战区", "城市站"}
        ...
        for dataset, context, score in candidate_contexts[:3]:
            ...
            if profile:
                ...
                explicit_level_hit = bool(
                    re.search(r"片区|战区|行业部|城市战区|城市站|客户经理|业务员", question)
                )
                alias_level_hit = any(
                    re.search(r"片区|战区|行业部|城市战区|城市站|客户经理|业务员", alias)
        ...
        # 守卫：如果问题包含层级词（如"战区"）且多个数据集都支持该层级，不允许 direct_execute 绕过确认
        ...
        if level_guard_blocks_direct_execute:
            # 强制确认：纯通用层级问题（没有事业群/数据集前缀）必须弹确认，不允许自动命中
    ...
    def _build_context_blob(self, context: Dict[str, Any]) -> str:
        ...
        if row_security.get("mode") == "org_tree":
            row_security_lines.append(
                ...
                "优先输出 节点名称、上级名称、战区、事业群、行业部、片区、组织路径、链接字段(勿删)；"
                "如源数据有编码，也输出组织编码。"
            )
            row_security_lines.append(
                "系统会在执行前二次套行级过滤；如果最终结果没有任何组织列，或客户经理明细缺少上级战区/片区/链路，普通用户可能查询不到数据。"
            )
            row_security_lines.append(
    ...
    def _build_rule_based_sql(self, question: str, route: Dict[str, Any], context: Dict[str, Any]) -> str:
        ...
        # 根节点别名 → 实际层级值映射（消费者/电商用"事业群总体"，商用/阶段一用"事业群"）
        if intent_target_level == "消费者事业群":
            actual_sy_level_value = "消费者事业群总体"
        elif intent_target_level == "商用事业群":
            actual_sy_level_value = "事业群"
        elif intent_target_level == "电商事业群":
            actual_sy_level_value = "电商事业群总体"
        else:
        ...
        generic_level_terms = {"战区", "片区", "行业部", "客户经理", "业务员", "事业群", "城市战区", "消费者事业群", "商用事业群", "电商事业群"}
        ...
        # 解决"商用四个战区的业绩"这种事实陈述被误判为ranking/混合层级的问题
        _quantity_verified_fact = False  # 标记：数量词事实验证通过，跳过ranking分支
        quantity_match_global = re.search(r"(三|四|五|六|七|八|九|十|两|\d+)(?:个|大|家|名|位)(片区|战区|行业部|客户经理|事业群|城市战区)", normalized_question)
        ...
        if intent_is_filter and query_intent.get("filter_metric_key") == "level_only":
            ...
            elif not is_multi_parent and "行业部" in normalized_question:
                where_parts.append("层级 = '行业部'")
            elif not is_multi_parent and "片区" in normalized_question:
                where_parts.append("层级 = '片区'")
            elif not is_multi_parent and "战区" in normalized_question:
                where_parts.append("层级 IN ('战区', '行业部')")
            if "行业线" in normalized_question:
                where_parts.append("线 = '行业线'")
            elif "区域线" in normalized_question:
                where_parts.append("线 = '区域线'")
            ...
            if entity_names:
                ...
                # bug 2026-08-26：泛称+层级实体（"商用战区"）分解为 层级+上级名称 过滤，避免 IN 伪节点名 0 行
                ...
                if specific_names:
                    ...
                    if intent_target_level == "客户经理" and not is_multi_parent:
                        where_parts.append(f"客户经理 IN ({quoted_names})")
                    else:
...
FROM 汇总结果
WHERE {where_clause}
ORDER BY 达成率 DESC, 剩余任务金额 DESC, 线 DESC, 节点名称
LIMIT 200
""".strip()
        ...
        if intent_is_filter and filter_metric_column in allowed_filter_columns and filter_operator in {"<", "<=", ">", ">=", "=", "between"} and filter_value is not None:
            ...
            if filter_value_sql:
                ...
                if not (matched_triggers & spoken_filter_triggers):
                    level_values = {"战区", "片区", "行业部", "事业群", "城市站", "城市战区", "客户经理"}
                    ...
                    all_entities = [
                        ...
                        and len(n) >= 4
                        and any(n.endswith(suffix) for suffix in ["战区", "片区", "行业部", "事业群", "城市站", "城市战区"])
                        and not re.search(r"\d|万", n)
                        # 数据集根节点名（如 confirm 注入的"组织树标准名称：商用事业群"）是路由信息不是查询实体，
                        # 误注入会把过滤范围劫持成"上级=商用事业群的全部节点"（bug 2026-08-25 追踪确认）
                    ...
                    # bug 2026-08-26：泛称+层级实体（如"商用战区"="商用"+"战区"）不是真实节点名，
                    # 直接进 节点名称 IN (...) 必然 0 行。分解为 层级+上级名称 过滤，
                    # 返回该节点下该层级全部子节点（"商用战区"→商用事业群下 4 家战区）。
                    if entity_names:
                        _level_scope_parts = []
                        _name_entities = []
                    ...
                    if entity_names:
                        ...
                        if target_level == "战区":
                            has_business_unit = any(e.endswith("事业群") for e in entity_names)
                            has_branch = any(e.endswith("战区") and not e.endswith("城市战区") for e in entity_names)
                            if has_business_unit:
                                where_parts.append(f"上级名称 IN ({quoted_entities})")
                            elif has_branch:
                                where_parts.append(f"节点名称 IN ({quoted_entities})")
                        elif target_level == "城市战区" or target_level == "城市站":
                ...
                tie_breaker = "剩余任务金额 DESC, 线 DESC, 节点名称" if filter_metric_column == "达成率" else "达成率 ASC, 线 DESC, 节点名称"
                generated_sql = f"""
WITH 汇总结果 AS (
{syyb_base_sql}
)
...
""".strip()
        ...
        if intent_is_comparison:
            ...
            if left_col and right_col:
                ...
                if intent_target_level:
                    where_parts.insert(0, f"层级 = '{actual_sy_level_value}'")
                elif "片区" in normalized_question:
                    where_parts.insert(0, "层级 = '片区'")
                elif "客户经理" in normalized_question:
                    where_parts.insert(0, "层级 = '客户经理'")
                elif "战区" in normalized_question or "行业部" in normalized_question:
                    where_parts.insert(0, "层级 IN ('战区', '行业部')")
                where_clause = " AND ".join(where_parts)
                return f"""
WITH 汇总结果 AS (
{syyb_base_sql}
...
FROM 汇总结果
WHERE {where_clause}
ORDER BY {left_col} DESC, 线 DESC, 节点名称
LIMIT 200
""".strip()
        ...
        if intent_is_aggregate:
            ...
            if not group_level:
                if "客户经理" in normalized_question:
                    group_level = "客户经理"
                elif "片区" in normalized_question:
                    group_level = "片区"
                elif "战区" in normalized_question or "行业部" in normalized_question:
                    group_level = "战区"
                elif "线" in normalized_question:
                    group_level = "线"
                else:
                    group_level = "战区"

            # 数量词事实验证通过（如"四个战区"），按基线口径返回该层级明细+一级下级
            if _quantity_verified_fact:
                where_conditions = [f"层级 = '{group_level}'"]
                # 战区层级额外过滤名称含"战区"，排除行业部
                if group_level == "战区":
                    where_conditions.append("节点名称 LIKE '%战区'")
                where_clause = " AND ".join(where_conditions)
                dataset_display_name = dataset_name.replace("开单金额","").replace("（阶段一升级版）","").strip()
                # 只有带"整体/盘点/区域/总览"等盘点词的才按 29 行口径（bug 2026-08-21）；
                # 裸数量词（如"商用的四个战区"）保持 bug#19/20 已确认的汇总行+明细 comparative 布局。
                _pandian_overview = bool(re.search(r"整体|盘点|总览|全景|区域", normalized_question))
                if group_level == "战区" and _pandian_overview:
                    # 战区事实盘点（如"四大战区区域业绩整体盘点"）基线口径=29行：
                    # 4 战区 + 其一级下级 25 片区；不带客户经理、不拼伪事业群汇总行
                    # （汇总行仅含区域线 3.1 亿却标称事业群 4.55 亿会误导；KPI 由 report_spec 按根节点合计）。
                    return f"""
WITH 明细结果 AS (
{syyb_base_sql}
WHERE (层级 = '战区' AND 节点名称 LIKE '%战区')
   OR (层级 = '片区' AND 上级名称 LIKE '%战区')
)
SELECT 线, 层级, 节点名称, 上级名称, 事业群, 战区, 片区, 行业部, 客户经理, 总任务金额, 年度开单金额, 达成率, 剩余任务金额
FROM 明细结果
ORDER BY
  CASE 层级
    WHEN '战区' THEN 1
...
SELECT 线, 层级, 节点名称, 上级名称, 事业群, 战区, 片区, 行业部, 客户经理, 总任务金额, 年度开单金额, 达成率, 剩余任务金额 FROM (
    SELECT
        MAX(线) AS 线,
        '事业群' AS 层级,
        '{dataset_display_name}' AS 节点名称,
        '商用事业群' AS 上级名称,
        '{dataset_display_name}' AS 事业群,
        '' AS 战区,
        '' AS 片区,
        '' AS 行业部,
        '' AS 客户经理,
...
""".strip()
            ...
            if group_level == "线":
                group_by_field = "线"
                where_level = "线 IN ('区域线', '行业线')"
            elif "下属" in normalized_question and "片区" in normalized_question:
                group_by_field = "上级名称"
                where_level = "层级 = '片区'"
...
""".strip()
        ...
        if is_phase1_dataset and has_ranking_intent:
            ...
            asks_grouped_rank = any(token in normalized_question for token in ["各", "每个", "分别", "各自", "按上级", "按片区", "分组"])
            ...
            target_is_business_person = normalized_target_level in {"客户经理", "业务员", "销售", "销售人员"}
            target_is_office = normalized_target_level == "片区"
            target_is_business_dept = normalized_target_level == "行业部"
            target_is_branch = normalized_target_level == "战区"

            if target_is_business_person or "客户经理" in normalized_question or "业务员" in normalized_question:
                ...
                business_dept_extract = (
                    "TRIM(COALESCE(CASE WHEN jsonb_typeof(fields->'行业部') = 'array' THEN fields->'行业部'->0->>'text' ELSE fields->>'行业部' END, '')) AS 行业部"
                    if "行业部" in syyb_dictionary_keys
                    else "'' AS 行业部"
                )
...
客户经理汇总 AS (
    SELECT
        ...
        '客户经理' AS 层级,
        客户经理 AS 节点名称,
        COALESCE(NULLIF(片区, ''), NULLIF(行业部, ''), NULLIF(战区, ''), '商用事业群') AS 上级名称,
        NULLIF(战区, '') AS 战区,
        NULLIF(片区, '') AS 片区,
        NULLIF(行业部, '') AS 行业部,
        concat_ws(' / ', NULLIF(战区, ''), NULLIF(片区, ''), NULLIF(行业部, ''), NULLIF(客户经理, '')) AS 组织路径,
        SUM(总任务金额) AS 总任务金额,
        SUM(年度开单金额) AS 年度开单金额
    FROM 客户经理原始
    WHERE 客户经理 <> ''
    GROUP BY 战区, 片区, 行业部, 客户经理
),
客户经理结果 AS (
    SELECT
        线,
...
""".strip()
                ...
                # （bug 2026-08-25 用户实测"没有业绩的客户经理有哪些"误判 ranking 后 0 行）
                # 与片区分支（line ~5310）对齐：0 → 返回全量排名
...
""".strip()
            if target_is_office or "片区" in normalized_question:
...
{rank_limit_clause}
""".strip()
                intra_rank_where = f"战区内排名 <= {rank_limit}" if rank_limit > 0 else "TRUE"
                return f"""
WITH 汇总结果 AS (
{syyb_base_sql}
),
...
ORDER BY 上级名称, 战区内排名, {phase1_metric_column} {order_direction}, 剩余任务金额 DESC, 节点名称
LIMIT 10000
""".strip()
            if target_is_business_dept or "行业部" in normalized_question:
                return self._build_ranked_select_sql(
                    source_cte=f"WITH 汇总结果 AS (\n{syyb_base_sql}\n)",
                    source_name="汇总结果",
                    output_cte="行业部排序",
            ...
            if target_is_branch or "战区" in normalized_question:
                return self._build_ranked_select_sql(
                    source_cte=f"WITH 汇总结果 AS (\n{syyb_base_sql}\n)",
                    source_name="汇总结果",
                    output_cte="战区排序",
                    where_clause="层级 = '战区' AND 节点名称 LIKE '%战区'",
                    metric_column=phase1_metric_column,
                    direction=order_direction,
                    rank_limit=rank_limit,
                    rank_sides=rank_sides,
        ...
        if is_phase1_dataset and intent_name == "ranking":
            ...
            # 修复：目标层级如果匹配到 analysisDimensions 的根节点名称（如"商用事业群"），
            # 不能直接按层级字段过滤（层级字段值是"事业群"），应清空后按问题中的其他层级词兜底。
            ...
            # bug#3 修复：根节点 + 整体/总体/汇总/全部 词 → level_overview 应返回"事业群+战区+行业部"三层
            ...
            if _is_root_overview:
                # 消费者数据集根节点层级值是"消费者事业群总体"，商用是"商用事业群"，需按数据集区分
                if is_phase1_dataset or is_syyb_dataset:
                    root_overview_where = "层级 IN ('事业群','战区','行业部')"
                else:
                    root_overview_where = "层级 IN ('消费者事业群总体','战区','行业部')"
            ...
            if _is_root_overview:
                ...
                if verified_level == "战区":
                    where_clause = "层级 = '战区' AND 节点名称 LIKE '%战区'"
                elif verified_level == "城市战区":
                    where_clause = "层级 = '城市战区'"
                else:
                ...
                where_clause = "层级 = '片区'"
            elif "战区" in normalized_question and "行业部" not in normalized_question:
                where_clause = "层级 = '战区' AND 节点名称 LIKE '%战区'"
            elif "行业部" in normalized_question and "战区" not in normalized_question:
                where_clause = "层级 = '行业部'"
            elif "战区" in normalized_question and "行业部" in normalized_question:
                where_clause = "层级 IN ('战区', '行业部')"
            elif "客户经理" in normalized_question or "人" in normalized_question:
                where_clause = "层级 = '客户经理'"
            else:
                # 默认按战区层级排序，避免返回全层级导致排序样本杂乱
                # 同时排除被额外映射为战区的行业部叶子节点，保证排名口径为区域战区
                where_clause = "层级 = '战区' AND 节点名称 LIKE '%战区'"
            if "区域线" in normalized_question:
                where_clause = f"{where_clause} AND 线 = '区域线'" if where_clause != "TRUE" else "线 = '区域线'"
            elif "行业线" in normalized_question:
                where_clause = f"{where_clause} AND 线 = '行业线'" if where_clause != "TRUE" else "线 = '行业线'"
...
""".strip()
        ...
        # 未明确点名具体节点时，跳过本地正则兜底，避免把“大于一个亿的战区”等当成实体。
        ...
        if not entity_names and has_specific_node:
            for match in re.findall(r"[\u4e00-\u9fa5A-Za-z0-9（）()]+?(?:片区|战区|行业部)", normalized_question):
                cleaned = match.strip("，,、 和与及的业绩情况表现")
                if re.search(r"\d|万", cleaned):
                    continue
                if is_phase1_dataset and cleaned in {"哪些片区", "各片区", "所有片区", "哪些战区", "各战区", "所有战区", "哪些行业部", "各行业部"}:
                    continue
        ...
        # P1防御性修复：实体合法性校验，只过滤明确的伪实体（前缀+层级词拼接，如"商用战区"），
        ...
        # 如果实体名只是通用层级词（如"战区"），按该层级过滤而不是按节点名过滤
        resolved_subject_name = str(query_intent.get("subject_name") or "").strip()
        generic_level_only = (
        ...
        if entity_names:
            quoted_entities = ",".join("'" + item.replace("'", "''") + "'" for item in entity_names)
            # 修复：问题中包含下钻词，或询问某组织下特定层级（如“战区片区业绩”）时，
            ...
            # 例如"衡原片区的业绩"中的"片区"是实体名一部分，不是要查询下级客户经理
            asks_child_level = False
            for level_word in ["片区", "行业部", "客户经理", "业务员"]:
            ...
            if comparison_intent:
                ...
                if cmp_profile:
                    for level in cmp_profile.get("levels") or []:
                        if str(level.get("dimension_name") or "").strip() in {"业务员", "客户经理"}:
                ...
                if cmp_org_names and cmp_person_names:
                    # 对比同时含组织与人名：分别按组织字段和客户经理返回
                    org_where_parts = []
                    for org in cmp_org_names:
                        safe_org = org.replace("'", "''")
                        if org.endswith("事业群"):
                            org_where_parts.append(f"事业群 = '{safe_org}'")
                        elif org.endswith("战区"):
                            org_where_parts.append(f"战区 = '{safe_org}'")
                        elif org.endswith("片区"):
                            org_where_parts.append(f"片区 = '{safe_org}'")
                        elif org.endswith("行业部"):
                            org_where_parts.append(f"行业部 = '{safe_org}'")
                        else:
                            org_where_parts.append(f"节点名称 = '{safe_org}' OR 上级名称 = '{safe_org}'")
                    quoted_persons = ",".join("'" + item.replace("'", "''") + "'" for item in cmp_person_names)
                    cmp_where_parts.append(f"({' OR '.join(org_where_parts)} OR 客户经理 IN ({quoted_persons}))")
                elif cmp_org_names:
                    org_where_parts = []
                    for org in cmp_org_names:
                        safe_org = org.replace("'", "''")
                    ...
                    quoted_persons = ",".join("'" + item.replace("'", "''") + "'" for item in cmp_person_names)
                    cmp_where_parts.append(f"客户经理 IN ({quoted_persons})")
...
""".strip()
            ...
            terminal_person_members.update(self._node_index_members_by_level(context, {"业务员", "客户经理"}))
            terminal_person_names = [e for e in drilldown_entities if e in terminal_person_members]
            is_terminal_node = (
                target_level_hint in {"客户经理", "业务员"}
                or "客户经理" in normalized_question
                or "业务员" in normalized_question
                or any(name.endswith("客户经理") or name.endswith("业务员") for name in drilldown_entities)
                or bool(terminal_person_names)
            )

            if is_terminal_node:
                # 区分客户经理人名与组织节点名：人名按客户经理字段过滤，组织节点按节点/上级过滤
                profile = get_dataset_profile(dataset_code, dataset_name)
                ...
                person_members.update(self._node_index_members_by_level(context, {"业务员", "客户经理"}))
                ...
                if mixed_with_compare:
                    ...
                    # 整组 OR 加括号，防 AND 拼接后被 SQL 优先级拆开
                    where_parts.append(f"(节点名称 IN ({quoted_all}) OR 上级名称 IN ({quoted_all}) OR 客户经理 IN ({quoted_all}))")
                else:
                    if org_names:
                        org_where_parts = []
                    ...
                    if person_names:
                        ...
                        where_parts.append(f"客户经理 IN ({quoted_persons})")
...
""".strip()
        ...
        if block_single_entity_shortcuts:
            ...
            quoted_entities = ",".join("'" + item.replace("'", "''") + "'" for item in drilldown_entities)
            target_level_hint = str(query_intent.get("target_level") or "").strip()
            is_terminal_node = target_level_hint == "客户经理" or any(name.endswith("客户经理") for name in drilldown_entities)
...
""".strip()
        ...
        if "商用事业群" in normalized_question and "整体达成率" in normalized_question:
            # 口径修正（bug 2026-08-21/22）：与 baseline §1.1/§1.4 对称——根节点总览带一级下级：
            # 事业群 1 行 + 4 战区 + 3 行业部 = 8 行。
            # 各行数值直接来自 datapivot_core_data 对应层级的自身行（该表每个层级都冗余完整总额），
...
SELECT *
FROM 汇总结果
WHERE 层级 IN ('事业群', '战区', '行业部')
ORDER BY
  CASE 层级 WHEN '事业群' THEN 0 WHEN '战区' THEN 1 WHEN '行业部' THEN 1 ELSE 9 END,
  达成率 DESC NULLS LAST,
  节点名称
LIMIT 100
""".strip()

        # 兜底：纯通用层级词（如"战区"）按层级返回所有节点
...
""".strip()
    ...
    def _build_online_sql(self, normalized_question: str, context: Dict[str, Any]) -> str:
        ...
        # resolve_member_mentions 可能因同时命中事业群和业务承接角色而误判为 comparison。
        ...
        # 用户层级 -> 实际层级 + 投影模式
        # 电商视图物理层级只有 事业群/行业部/客户经理；
        # 客户经理行同时承载「业务承接角色」（细分业务）和「承接人」（负责人）两个逻辑层级，二者是同一物理行。
        USER_LEVELS = {
            "事业群": {"actual": "事业群", "mode": "segment"},
            "行业部": {"actual": "行业部", "mode": "segment"},
            "业务承接角色": {"actual": "客户经理", "mode": "segment"},
            "细分业务": {"actual": "客户经理", "mode": "segment"},
            "业务线": {"actual": "客户经理", "mode": "segment"},
            "承接人": {"actual": "客户经理", "mode": "manager"},
            "任务承接人": {"actual": "客户经理", "mode": "manager"},
            "负责人": {"actual": "客户经理", "mode": "manager"},
            "客户经理": {"actual": "客户经理", "mode": "manager"},
            # 兜底：消费者/商用常用的“客户经理”在电商口语里对应负责人层级
            "客户经理": {"actual": "客户经理", "mode": "manager"},
        }
        ...
        def infer_user_level() -> str:
            ...
            # 根节点问法（如“电商事业群的业绩”）默认展示直接下级行业部，
            # 避免只返回事业群汇总单行导致左侧卡片没有下级列表。
            if (
                intent_target_level == "事业群"
                and ("电商事业群" in q or "电商" in q)
            ...
            if focus_dimension and focus_dimension != "事业群":
            ...
            if "承接人" in q or "负责人" in q or "任务承接人" in q or "客户经理" in q:
                return "承接人"
            if "业务承接角色" in q or "细分业务" in q or "业务线" in q:
                return "业务承接角色"
            if "行业部" in q:
                return "行业部"
            if ("电商事业群" in q or "电商" in q) and not any(t in q for t in ["整体", "总体", "总览", "汇总", "全部"]):
                return "行业部"
            if "事业群" in q or "整体" in q or "全部" in q:
                return "事业群"
            return "业务承接角色"
        ...
        # “电商事业群根节点问法”的默认路径（聚焦事业群 → 返回行业部下钻列表）。
        ...
        level_cfg = USER_LEVELS.get(user_level, {"actual": "客户经理", "mode": "segment"})
        actual_level = level_cfg["actual"]
        projection_mode = level_cfg["mode"]

        # 投影列：
        # - manager 模式把「承接人/负责人」作为节点，行业部/事业群作为上级；
        # - segment 模式把「业务承接角色」作为节点，行业部/事业群作为上级；
        # - 默认返回时，客户经理行统一展示为「业务承接角色」，避免把业务承接角色误标成客户经理。
        ...
        if projection_mode == "manager":
            select_cols = """
                ...
                CASE
                    WHEN 层级级别 = '事业群' THEN NULL
                    WHEN 层级级别 = '行业部' THEN '电商事业群'
                    ELSE COALESCE(NULLIF(TRIM(行业部), ''), '电商事业群')
                END AS 上级名称,
                年度目标营收 AS 总任务金额,
                年度开单金额 AS 年度开单金额,
            ...
            select_cols = """
                '电商业务' AS 线,
                '业务承接角色' AS 层级,
                COALESCE(NULLIF(TRIM(细分业务), ''), NULLIF(TRIM(行业部), ''), '电商事业群') AS 节点名称,
                CASE
                    WHEN 层级级别 = '事业群' THEN NULL
                    WHEN 层级级别 = '行业部' THEN '电商事业群'
                    ELSE COALESCE(NULLIF(TRIM(行业部), ''), '电商事业群')
            ...
            select_cols = """
                '电商业务' AS 线,
                CASE
                    WHEN 层级级别 = '客户经理' AND NULLIF(TRIM(细分业务), '') IS NOT NULL THEN '业务承接角色'
                    ELSE 层级级别
                END AS 层级,
                COALESCE(NULLIF(TRIM(细分业务), ''), NULLIF(TRIM(行业部), ''), '电商事业群') AS 节点名称,
                CASE
        ...
        if intent == "comparison" or is_comparison:
            ...
            if compare_members:
                ...
                if compare_dimension == "行业部":
                    where_parts.append(f"行业部 IN ({quoted_members})")
                    ...
                    # 兜底：在节点名称里匹配
                    where_parts.append(
                        f"COALESCE(NULLIF(TRIM(细分业务), ''), NULLIF(TRIM(行业部), ''), '电商事业群') IN ({quoted_members})"
                    )
                    if actual_level:
                        where_parts.append(f"层级级别 = '{actual_level}'")
                # 人名对比需要把负责人作为节点
                if compare_dimension in {"承接人", "任务承接人", "负责人", "客户经理"}:
                    select_cols = """
                        '电商业务' AS 线,
                        '承接人' AS 层级,
                        COALESCE(NULLIF(TRIM(负责人), ''), '未知承接人') AS 节点名称,
        ...
        # 排名
            ...
            if actual_level:
                where_parts.append(f"层级级别 = '{actual_level}'")
            # 如果聚焦到某个行业部，且不是行业部自身排名，则限定子树
            if focus_member and focus_dimension == "行业部" and user_level != "行业部":
                where_parts.append(f"组织路径 LIKE {quote('电商事业群;' + focus_member + '%')}")
        ...
        # 下钻 / 明细 / 总览（聚焦单个节点）
            ...
            if focus_dimension == "事业群":
                where_parts.append(f"组织路径 LIKE {quote('电商事业群%')}")
                if user_level == "行业部":
                    where_parts.append("层级级别 = '行业部'")
            elif focus_dimension == "行业部":
                where_parts.append(f"(组织路径 LIKE {quote('电商事业群;' + focus_member + '%')} OR (行业部 = {quote(focus_member)} AND 层级级别 = '行业部'))")
            elif focus_dimension in {"业务承接角色", "细分业务", "业务线"}:
                where_parts.append(f"细分业务 = {quote(focus_member)}")
            elif focus_dimension in {"承接人", "任务承接人", "负责人", "客户经理"}:
                # 修复：不能硬编码"层级级别='客户经理'"。周望舒的节点索引 node_level='承接人'，
                # 但数据库里周望舒的层级级别='行业部'（行业部级的负责人）。
    ...
    def _build_consumer_business_sql(self, normalized_question: str, context: Dict[str, Any]) -> str:
        ...
        city_field_key = consumer_key("城市战区")
        city_field_expr = f"COALESCE(NULLIF(TRIM(fields->>'{city_field_key}'), ''), '')"
        query_intent = self._safe_dict(context.get("query_intent"))
        generic_level_terms = {"战区", "片区", "行业部", "客户经理", "业务员", "城市战区", "城市站", "事业群"}
        ...
        if intent_target_level in {"城市站", "城市战区"}:
            intent_target_level = "城市战区"
            query_intent["target_level"] = "城市战区"
        # ranking top1 时若 target_level 是 root level（事业群级）或为空，自动降到「战区」
        ...
        if (intent_is_ranking or query_intent.get("intent") == "ranking") and self._safe_dict(context.get("query_intent")).get("top_n") == 1:
            ...
            root_levels = {"消费者事业群", "商用事业群", "电商事业群", "事业群", "事业群总体", ""}
            if intent_target_level in root_levels or not intent_target_level.strip():
                intent_target_level = "战区"
                query_intent["target_level"] = "战区"
        asks_branch_extremes = (
            "战区" in normalized_question
        ...
        def normalize_consumer_entity_name(value: str) -> str:
            ...
            if text in {"城市站", "城市战区", "战区", "事业群"}:
                return ""
            if text.endswith("城市战区"):
                return text[:-5] + "城市站"
            return text
        ...
        if not entity_names and not asks_branch_extremes:
            for match in re.findall(r"[\u4e00-\u9fa5A-Za-z0-9（）()]+?(?:城市战区|城市站|战区|事业群)", normalized_question):
                cleaned = match.strip("，,、 和与及的业绩情况表现整体")
                normalized = normalize_consumer_entity_name(cleaned)
                if re.search(r"\d|万", normalized):
                    continue
                # ranking 修饰词不是实体名，跳过，避免"看下前三的城市战区"被当成实体触发空结果防御
                if re.search(r"前|后|排名|排行|最高|最低|最好|最差|top|第", normalized, flags=re.I):
                    continue
                if normalized and normalized not in {"哪些战区", "各战区", "所有战区", "哪些城市站", "各城市站", "所有城市站"}:
                    entity_names.append(normalized)
        entity_names = list(dict.fromkeys(entity_names))

        # P1防御性修复：实体合法性校验，只过滤明确的伪实体（前缀+层级词拼接，如"消费者战区"），
        # 保留所有其他实体（包括跨数据集真实实体，交给后续路由逻辑自动切换数据集）
        # 修复：取所有数据集节点并集，避免多轮hint沿用导致跨数据集合法实体被误剔
        if entity_names:
            valid_node_names = set()
        ...
        if entity_names and not generic_level_only:
            ...
            if current_ds_id and not any(self._is_entity_in_dataset(n, current_ds_id) for n in entity_names):
                return ("SELECT '消费者经营链路' AS 线, '全部' AS 分析口径, "
        ...
        scope_filter = ""
        # 关键修复：ranking 意图不应被实体 scope_filter 污染（避免"谁业绩最好"被"消费者事业群"实体带成全量）
...
"""
        ...
        # 根节点别名 vs 实际层级值的映射：消费者/电商/阶段一用"事业群总体"，商用用"事业群"
        if not scope_filter and intent_target_level:
            if is_root_overview:
                _lvls = "','".join(LEVEL_OVERVIEW_WITH_CHILDREN[intent_target_level])
                scope_filter = f"WHERE 层级 IN ('{_lvls}')"
            elif intent_target_level == "消费者事业群":
                scope_filter = "WHERE 层级 = '消费者事业群总体'"
            elif intent_target_level == "商用事业群":
                scope_filter = "WHERE 层级 = '事业群'"
            else:
                scope_filter = f"WHERE 层级 = '{intent_target_level}'"
...
""".format(
        ...
        city_level_requested = (
            intent_target_level in {"城市战区", "城市站"}
            or "城市战区" in normalized_question
            or "城市站" in normalized_question
        )
        asks_best_branch = (
            "战区" in normalized_question
        ...
        asks_branch_ranking = (
            ...
            and (
                (intent_is_ranking and intent_target_level == "战区")
                or (
                    "战区" in normalized_question
        ...
        def consumer_rank_limit() -> int:
            ...
            if rank_limit <= 0 and asks_best_branch:
                rank_limit = 1
            if rank_limit <= 0:
                # bug#15：无数量排名（如"消费者城市战区排名"）应返回全量，而非截断20条。
        ...
        if intent_is_comparison:
            ...
            if left_col and right_col:
                ...
                if intent_target_level:
                    ...
                    where_parts.append("层级 = '城市战区'")
                elif "战区" in normalized_question and "城市战区" not in normalized_question:
                    where_parts.append("层级 = '战区'")
...
""".strip()
        ...
        if intent_is_aggregate:
            group_level = intent_target_level
            if not group_level:
                if "战区" in normalized_question and "城市战区" not in normalized_question:
                    group_level = "战区"
                elif "城市战区" in normalized_question:
                    group_level = "城市战区"
                else:
                    group_level = "战区"

            # 数量词事实验证通过（如"四个战区"），直接返回明细数据+整体汇总行，供顶部KPI使用
            if query_intent.get("_quantity_verified"):
                where_conditions = [f"层级 = '{group_level}'"]
                # 城市战区层级额外过滤名称含"战区"
                if group_level == "城市战区" or group_level == "战区":
                    where_conditions.append("节点名称 LIKE '%战区'")
                where_clause = " AND ".join(where_conditions)
...
SELECT 线, 分析口径, 层级, 节点名称, 上级名称, 总任务金额, 年度开单金额, 达成率, 剩余任务金额, 线下任务_万元, 新零售任务_万元, 燃气定制任务_万元, 地产任务_万元, 线下实际_万元, 新零售实际_万元, 燃气定制实际_万元, 地产实际_万元 FROM (
    SELECT
        MAX(线) AS 线,
        MAX(分析口径) AS 分析口径,
        '事业群' AS 层级,
...
""".strip()
            ...
            group_by_field = "节点名称"
            where_level = f"层级 = '{group_level}'"
            if "下属" in normalized_question and "城市战区" in normalized_question:
                group_by_field = "上级名称"
                where_level = "层级 = '城市战区'"

            # 统计各战区拥有的城市战区数量
            if ("各战区" in normalized_question and "城市战区数量" in normalized_question) or "各战区拥有的城市战区数量" in normalized_question:
                group_by_field = "上级名称"
                where_level = "层级 = '城市战区'"
                agg_select = "COUNT(*) AS 城市战区数量"
                order_by = "城市战区数量 DESC NULLS LAST"
            # 单实体 + "各项指标" 返回该节点全量指标
...
""".strip()
        ...
        if asks_branch_ranking and not asks_branch_extremes and not (channel_metric and asks_best_branch):
            ...
            # ranking top1 时，若 target_level 是 root level（事业群级）或为空，自动降到「战区」
            root_levels = {"消费者事业群", "商用事业群", "电商事业群", "事业群", "事业群总体"}
            if rank_limit == 1 and rank_sides != "both" and (intent_target_level in root_levels or not intent_target_level):
                intent_target_level = "战区"
            explicit_target = intent_target_level in ["战区", "城市战区"]
...
""".strip()
            ...
            # 原有逻辑：用户没有明确层级时，才返回「战区 + 城市战区」上下级数据
            if rank_sides == "both":
                return self._build_ranked_select_sql(
                    source_cte=base_sql,
                    source_name="汇总结果",
...
INNER JOIN (
    ...
    SELECT DISTINCT 节点名称, 层级, 上级名称 FROM 汇总结果
    WHERE 层级 = '战区'
       OR 上级名称 IN (SELECT 节点名称 FROM 汇总结果 WHERE 层级 = '战区')
) allowed_nodes
  ON r.节点名称 = allowed_nodes.节点名称 AND r.层级 = allowed_nodes.层级
LEFT JOIN 排名战区 b
  ON (r.层级 = '战区' AND r.节点名称 = b.节点名称)
ORDER BY
    CASE WHEN b.排名序号 IS NOT NULL THEN 0 ELSE 1 END,
    b.排名序号,
    CASE r.层级 WHEN '战区' THEN 1 ELSE 2 END,
    r.{sort_column} {order_direction},
    r.年度开单金额 DESC,
    r.剩余任务金额 DESC,
    r.节点名称
...
SELECT *
FROM 汇总结果
WHERE 节点名称 IN (SELECT 节点名称 FROM 最佳战区)
   OR 上级名称 IN (SELECT 节点名称 FROM 最佳战区)
ORDER BY
...
{scope_filter}
ORDER BY
    CASE 层级 WHEN '消费者事业群总体' THEN 0 WHEN '战区' THEN 1 WHEN '城市战区' THEN 2 ELSE 9 END,
    上级名称 NULLS FIRST,
    达成率 DESC,
    节点名称
LIMIT 10000
""".strip()
    ...
    def _agent2_generate_sql(
        ...
        force_grouped_ranking = (
            defer_rule_fallback
            and rule_based_sql
            and any(token in normalized_question for token in ["片区", "客户经理", "业务员"])
        ...
        force_descendant_scope_rule = (
            defer_rule_fallback
            and rule_based_sql
            and re.search(r"(?:片区|战区|行业部)", normalized_question)
            and any(token in normalized_question for token in ["下面", "下级", "客户经理", "业务员", "人员", "的人", "明细", "咋样", "怎么样"])
        )
        if (
            rule_based_sql
            and not seed_sql
...
3. 如果用户问题包含时间、组织、区域、战区等口径，必须在 SQL 中体现对应过滤或分组。
...
8. 如果用户问“哪些片区/客户经理最低、最差、完成不好、Top/排名”，且没有指定某个上级组织，必须按上级分组比较：
   - 片区必须按 战区/上级名称 分组，用 ROW_NUMBER() OVER (PARTITION BY 上级名称 ORDER BY 达成率 ASC/DESC...) 输出每个战区下的最低/最高片区，不能把所有片区全局混排。
   - 客户经理必须按片区或行业部上级分组比较，不能把所有客户经理全局混排。
...
"""
    ...
    def _followup_dataset_hint_from_memory(
        ...
        org_level_tokens = ("战区", "城市战区", "城市站", "行业部", "片区", "客户经理")
        ...
        if not self._profile_supports_level(profile, "战区"):
            return []
        ...
        for alias in target_dataset.get("synonyms") or []:
            ...
            if compact_alias not in normalized_question:
                continue
            if compact_alias in {"战区", "城市战区", "城市站", "行业部", "片区", "业绩", "排名"}:
                continue
    ...
    def _extract_followup_org_target(self, question: str) -> str:
        ...
        更弱（少 查询/我想知道/这位 等），且没有 contains+层级优先，导致在事业群前缀、倒桩等场景
    ...
    def _extract_bare_org_subject_by_node_index(self, question: str) -> str:
        ...
        # 精确匹配：cleaned 本身就是某个 alias（不接受前缀模糊匹配，避免 "邓景行看下人" 被当作主体返回）
        alias_names = {
        ...
        if len(cleaned) >= 2 and cleaned in alias_names:
            return cleaned
        # contains 匹配 + 层级优先（修 Bug C："商用事业群程允文"→程允文 而非 商用事业群）
        ...
        # 用于“低于30%的战区”这类带过滤条件的问题，确保能触发数据集确认
    ...
    def _pick_finer_subject(self, llm_subject: str, fallback_subject: str) -> str:
        ...
        修 LLM 把 "商用事业群程允文" 误判为 "商用事业群"（事业群层）而 fallback 正确提取 "程允文"（客户经理层）的问题。
        层级相同或无法判断时尊重 LLM。
        """
...
3. 如果问题里明确包含 片区/战区/行业部/事业群/客户经理/业务员/城市战区/城市站，要尽量保留层级词。
4. 数量词（如“三个”、“前3”、“几家”）不是组织主体的一部分，只保留层级/节点名称；例如“三个行业部”的主体是“行业部”，“前3战区”的主体是“战区”。
5. 如果无法稳定识别，subject_name 返回空字符串。
6. rewritten_question 只在识别黄维桢时输出，例如“衡原片区的业绩”、“行业部的业绩”。
...
""".strip()
        ...
        # 层级择优：LLM 返回上层组织单元（事业群/战区）而 fallback 提取到更细节点（客户经理/城市站）时，
        # 取更细层级——修 LLM 把 "商用事业群程允文" 误判为 "商用事业群" 的问题
        ...
        org_level_keywords = ("事业群", "战区", "片区", "行业部", "城市战区", "城市站")
    ...
    def _guard_followup_org_target_resolution(
        ...
        # 兜底：组织树可能只维护到战区层，但数据集画像/真实数据里已有片区、业务员等下级节点。
...
5. 修复后仍要满足报告配置要求：尽量输出 线、层级、节点名称、上级名称 以及关键指标列。
...
""".strip()
    ...
    def _agent3_review(
        ...
        if asks_channel_best_branch:
            rule_sql = self._build_rule_based_sql(question, route, context)
            if rule_sql and "最佳战区" in rule_sql:
                self._append_trace(
                    trace,
                    "agent3.sql_review.best_branch_rule_fix",
                    "info",
                    review_summary="分业务线最佳战区查询已改用稳定规则 SQL，避免复核阶段提前过滤下级节点。",
                    sql=self._truncate_text(rule_sql, 12000),
                )
                ...
                return {
                    "approved": True,
                    "final_sql": rule_sql,
                    "review_summary": "已改用稳定规则 SQL：先按业务线选最佳战区，再展示该战区及其下级节点。",
                    "risks": [],
                    "fixes": ["避免用总任务过滤最终展示结果，保留下级城市站节点。"],
                }
        if asks_branch_extreme_compare:
            rule_sql = self._build_rule_based_sql(question, route, context)
            if rule_sql and "头尾战区" in rule_sql:
                self._append_trace(
                    trace,
                    "agent3.sql_review.branch_extreme_rule_fix",
                    "info",
                    review_summary="战区最高/最低对比查询已改用稳定规则 SQL，避免复核阶段误用字段或改坏样例 SQL。",
                    sql=self._truncate_text(rule_sql, 12000),
                )
                validation = self._dataset_field_validation(rule_sql, context)
                if not validation.get("ok"):
                ...
                return {
                    "approved": True,
                    "final_sql": rule_sql,
                    "review_summary": "已改用稳定规则 SQL：返回达成率最高、最低战区，并对比任务体量、实际开单、缺口和达成率差距。",
                    "risks": [],
                    "fixes": ["使用当前年字段和标准金额字段，避免维护样例中的年份字段错误。"],
                }
        needs_descendant_rows = (
            re.search(r"(?:片区|战区|行业部)", normalized_question)
            and any(token in normalized_question for token in ["下面", "下级", "客户经理", "业务员", "人员", "的人", "明细", "咋样", "怎么样"])
        )
            ...
            if not (has_hierarchy_columns and has_descendant_filter):
                ...
                if rule_sql:
                    self._append_trace(
                        ...
                        "warning",
                        review_summary="问题要求查看下级人员，已改用稳定层级 SQL 返回命中节点和客户经理明细。",
                        sql=self._truncate_text(rule_sql, 12000),
                    )
                    validation = self._dataset_field_validation(rule_sql, context)
                    if not validation.get("ok"):
        ...
        if self._is_read_only_sql(sql_text) and "datapivot_core_data" in str(sql_text):
            validation = self._dataset_field_validation(sql_text, context)
            if not validation.get("ok"):
...
1. 动态布局：先识别意图。对比查询使用"核心结论 -> 关键指标对标 -> 战区级明细对比表 -> 层级差异核心看点 -> 落地建议"的对称结构；单体查询使用"核心 KPI -> 层级分布 -> 细分明细"的纵向结构；列表或排名查询突出名次、差距和相对领先/相对承压节点。
2. 强制格式化：所有金额必须按统一函数口径表达：1万以下原样；1万-100万保留1位小数并使用"万"；100万-1亿取整"万"；1亿以上保留2位小数"亿"。不得随意生成金额格式。
3. 问题优先：核心结论第一句话必须直接回答用户原问题。用户问"哪个战区/行业部/片区最好、最高、最低、最差"时，先回答目标管理层级的对象名称，再给达成率、总任务、实际开单、任务缺口；下级城市站/片区只能作为后续支撑，不能抢在目标层级结论前面。
...
"""
        try:
            ...
            # 自动明细对比表（2026-09-06 用户反馈"按战区分两列展示"）：Agent4 LLM 即使强约束也不稳定输出表格，
    ...
    def _run_pipeline(
        ...
        for dataset_id in dataset_ids:
            ...
            # 如果问题含并列多个人名（如“苏观澜和钟自蹊的业绩”），优先用规则提取的多人，
            # 避免 Agent1 单主体解析把其中一个人覆盖掉。
            # bug#6：必须用 raw_question 提取，refined_query 可能已被 Agent1 重写为单主体（如"苏观澜的业绩"）。
            ...
            # 把"谁业绩最好"剥成"消费者事业群"而丢失"最好"这个 ranking 信号。
            ...
            if is_followup_rewritten and raw_question:
                ...
                # 主体已纠正（如"商用事业群程允文"→"程允文"）：以 refined_query 为主体，避免原始问题里的事业群层级词误导 target_level
            ...
            if coerced_final_sql != final_sql:
                ...
                review = {
                    **review,
                    "final_sql": final_sql,
                    "review_summary": ((review.get("review_summary") or "") + " 已按原始问题的城市站层级修正 SQL。").strip(),
                }
                self._append_trace(
                    trace,
    ...
    def ask(
        ...
        if len(explicit_dataset_ids) == 1 and effective_question != question:
            # 当前追问已明确点名唯一数据集/事业群时，不能把上一轮的事业群文本拼进来。
            # 否则会形成"消费者事业群 + 商用事业群"混合问题，Agent1 会误判为需要确认。
            # 但如果 effective_question 是主体纠正的结果（org_subject_resolution 有 rewritten_question），
            # 说明是"商用事业群程允文"→"程允文"这类纠正，不是追问拼接，不应重置。
        ...
        try:
            ...
            if preferred_dataset_ids:
                ...
                # 主体纠正后 effective_question 已是 "程允文的业绩"，
                # 路由层应基于纠正后的问题走，避免再用原始 "商用事业群程允文" 识别出事业群层级
    ...
    def confirm_by_boss(
        ...
        if selected_option_item:
            ...
            if not resolved_subject_name:
                ...
                # （如"消费者事业群 · 垫底的三个战区"）。通用提取取分隔符【后】段，会把
                # 问句尾巴（"垫底的三个战区"）当节点主体注入 route，触发规则引擎实体防御
            ...
            if resolved_subject_name:
                ...
                # bug#17：original_subject_name 是纯层级词（如"战区"）时不做替换，
                # 否则会把"各战区业绩排名"重写成"各消费者事业群业绩排名"，丢失 ranking 信号。
                _GENERIC_LEVEL_WORDS = {"战区", "片区", "行业部", "事业群", "城市战区", "城市站", "客户经理", "业务员"}
        ...
        if conversation_session_id and not result.get("error") and not result.get("early_clarify"):
            self.short_term_memory.remember_result(conversation_session_id, question, route, result)
        # P0-1：确认会话不一次即焚——首次 confirm 黄维桢后保留 pending payload，
