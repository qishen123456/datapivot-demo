...
def _alias_matches_question(alias: str, node_name: str, normalized_question: str) -> bool:
    ...
    # 若按普通冲突拒绝，"用服战区滤芯排名"的"用服"会被丢弃，路由被其他数据集的
...
class OrganizationRouteResolver:
    ...
    # "商用战区"不是真实节点名（实际节点是沧澜/凛澜/淼澜/漠澜战区），
    ...
    def resolve(
        ...
        # 而不是"选节点"（"用服浔澜"=用服事业群的浔澜战区）。把 mentions 收窄到
        # 该数据集并优先保留非根节点，避免"用服浔澜战区的业绩"被弹成跨数据集确认卡。
        # 单数据集场景不动（"商用事业群和沧澜战区对比"的根节点仍是有效成员）。
    ...
    def domain_prefix_scope_id(
        ...
        用服接入实测（2026-09-03）：LLM 主体纠正会把"用服浔澜"的域前缀"用服"
        当口语剥掉（改写为"浔澜战区的业绩"），路由只剩跨数据集同名节点，
