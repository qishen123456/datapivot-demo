...
def _clean_org_subject_candidate(value: str) -> str:
    ...
    # bug 2026-08-26："帮我查询下商用战区" → 旧正则里"查询"排在"查询下"前面，
    # 引擎先吃掉"查询"剩下裸"下"无法匹配任何备选 → 产出伪主体"下商用战区"。
    # 修复：备选按长度降序排列（长词优先），保证"查询下/查询一下"先于"查询"被消费。
    text = re.sub(
        r"^(?:麻烦帮我查下|麻烦帮我看下|继续看一下|继续查一下|我想知道|我想了解|继续看下|继续查下|查询一下|帮我看看|再看一下|看一下|查询下|分析一下|了解一下|我想看|我想查|我想问|查一下|继续看|继续查|分析下|了解下|看下|查下|问一下|问下|查询|帮忙|帮我|想看|想查|想问|麻烦|再看|请)+",
        "",
        text,
    ).strip()
...
def _looks_like_org_subject_question(question: str) -> bool:
    ...
    if not text:
        return False
    # 纯层级词（如"城市战区"）+ 业绩/情况，不应走主体追问，应直接按层级 Overview 处理
    level_only_terms = {"战区", "片区", "行业部", "城市战区", "城市站", "事业群", "客户经理", "业务员"}
    ...
    # 裸中文基数词/阿拉伯数字 + 量词（四大/三个/五家/三家公司/4个事业群）同样属于
    # 排名/限定计数，必须排除，否则"四大战区..."会被改写成"战区的业绩"丢掉"四"。
    ...
    if re.search(rf"[\u4e00-\u9fa5A-Za-z0-9（）()]{{2,}}(?:的|之下|下面|下属)?\s*({level_pattern})\s*$", text):
        return False
    has_org_level = bool(re.search(r"片区|战区|行业部|城市战区|城市站|事业群|客户经理|业务员", text))
